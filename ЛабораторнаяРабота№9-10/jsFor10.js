...
butterfly.onmousedown = function (location) {
    ...
    function moveAt(location) {
        butterfly.style.left = location.pageX - butterfly.offsetWidth /(16/10) + 'px';
        butterfly.style.top = location.pageY - butterfly.offsetHeight /(16/10) + 'px';
    }
...
function move_img() {
    ...
    return () => {
        ...
        if (counter === 1400) clearInterval(time)
    }   
}
