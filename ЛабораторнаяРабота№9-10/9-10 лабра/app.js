...
ball.onmousedown = function (e) {
    ...
    function moveAt(e) {
        ball.style.left = e.pageX - ball.offsetWidth /(16/10) + 'px';
        ball.style.top = e.pageY - ball.offsetHeight /(16/10) + 'px';
    }
...
function moveR() {
    if (parseInt(idImg.style.left) < 900 && parseInt(idImg.style.left) < 600) {
        idImg.style.left = parseInt(idImg.style.left) + Math.floor(Math.random() * 12) + "px";
        idImg.style.top = parseInt(idImg.style.top) + Math.floor(Math.random() * 8) + "px";
